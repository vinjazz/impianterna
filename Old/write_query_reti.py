...
def gestisci_file_g02_reti():
    # Trova tutti i file che iniziano con G02 e finiscono con .xml
    file_g02 = [f for f in
                os.listdir("\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+") if
    ...
    file_g02_percorso = [
        os.path.join("\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+", f) for f
        in file_g02]
...
def write_G03_MU_query_reti():
 ...
 where
   ...
   and t.cod_pratica in (
    ...
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G03_MU_Query_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query)
def move_files_reti():
    source_folder = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\LOG G03 E G04"
    destination_folder = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\LOG G03 E G04\\old"
...
def write_query_reti():
    ...
    with open(
            '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\G01\\G01.csv') as G01_csv_file:
...
{result_string}
    ...
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\Update_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query)
    webbrowser.open(filename)
def write_files_starting_with_G03_reti():
    # List all files in the directory
    folder_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\LOG G03 E G04"
    ...
    for file in g03_files:
        ...
        with open(
                f'\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\LOG G03 E G04\\{file}') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=';')
            line_count = 0
            for row in csv_reader:
...
"""
    ...
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G03_Update_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query_total)
        file.close()
    # webbrowser.open(filename)
def write_files_starting_with_G04_reti():
    folder_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\LOG G03 E G04"
    files_in_folder = os.listdir(folder_path)
    query_total = ''
...
quit;
    ...
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G04_Update_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query_total)
        file.close()
...
"""
    ...
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G01_update_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query)
        file.close()
def g12_query_reti():
...
"""
        ...
        now = datetime.datetime.now()
        timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
        filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G13_update_{timestamp}.txt"
        with open(filename, 'w') as file:
            file.write(query)
            file.close()
def g03_query_reti():
...
quit;"""
        ...
        with open(filename, 'w') as file:
            file.write(query)
            file.close()
def g01_controllo_query_reti():
    with open('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\G01\\G01.csv') as G01_csv_file:
        G01_lista = []
        G01_csv_reader = csv.reader(G01_csv_file, delimiter=';')
        G01_line_count = 0
        for row in G01_csv_reader:
...
"""
    ...
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\update_query\\G01_Controllo_update_{timestamp}.txt"
    with open(filename, 'w') as file:
        file.write(query)
        file.close()
def g12_controllo_query_reti():
    lista = []
    with open(
            '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\G12\\G12.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=';')
