...
def flussoG13reti():
    basepath = '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\4-Registrati\\'
    for entry in os.listdir(basepath):
        if os.path.isfile(os.path.join(basepath, entry)):
            name = entry
    xmlparse = Xet.parse('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\4-Registrati\\' + name)
    ...
    with open('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\G12\\G12.csv') as csv_file:
            ...
            for row in csv_reader:
                ...
                if line_count == 0:
                    ...
                    for item in rows:
                        ...
                        # tree.write(f"C:\\ImpiantiTerna\\G12\\G12_{row[7]}.xml", short_empty_elements=False)
                        #
                        else:
                            pass
                line_count += 1
                        # with open('C:\\ImpiantiTerna\\G12\\log', 'w', encoding='UTF8') as f:
                        #
                        #     writer = csv.writer(f)
                        #     writer.writerow(f'{date.today()} - {CODICE_RINTRACCIABILITA}')
                        #     f.close()
    ...
    tree = ET.ElementTree(root)
    #tree.write(f"C:\\ImpiantiTerna\\G13\\G13_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml", short_empty_elements=False)
    tree.write(
        f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\G13_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml",
        short_empty_elements=False)
    Move_file.move_file('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Reti+\\4-Registrati')

def log(preventivo, row):
    line = f'{date.today()};{preventivo}'
    file_uno = open(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\reti+\\G13\\log\\{preventivo}.csv", "w")
    file_uno.write(line)
    for data in row:
        file_uno.write(f"\n {data};")
    file_uno.close()
