...
def gestisci_file_g02():
    ...
    try:
        directory = "\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Duereti"
    ...
    try:
        ...
        with open(csv_path) as csv_file:
            ...
            output_root = ET.Element("VALIDA_IMPIANTI", COD_SERVIZIO="G02", COD_FLUSSO="0050", 
                                   TERNA_PIVA="05779661007", GESTORE_PIVA="13632560960")
            ...
            output_xml_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Duereti\\G02_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml"
