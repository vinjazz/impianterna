...
def flussoG01New():
    try:
        with open('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\G01\\G01.csv') as csv_file:
    ...
    tree = ET.ElementTree(root)
    tree.write(
        f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\G01_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml",
        short_empty_elements=False)

def flussoG01_g12():
    try:
        with open('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\G01\\G01_G12.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=';')
            line_count = 0
            root = ET.Element("GESTIONE_POD", COD_SERVIZIO="G01", COD_FLUSSO="0050", TERNA_PIVA="05779661007",
                              GESTORE_PIVA="12883450152")
...
def log(preventivo, row):
    ...
    file_uno = open(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\G01\\log\\{preventivo}.csv", "w")
    file_uno.write(line)
    for data in row:
        file_uno.write(f"\n {data};")
    file_uno.close()
