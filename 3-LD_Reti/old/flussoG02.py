...
def gestisci_file_g02():
    ...
    file_g02 = [f for f in os.listdir("\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti") if f.startswith('G02') and f.endswith('.xml')]
    ...
    file_g02_percorso = [os.path.join("\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti", f) for f in file_g02]
...
def g02_query():
    lista = []
    with open(
            '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G12\\G12.csv') as csv_file:
...
quit;"""
        ...
        filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\update_query\\G02_update_{timestamp}.txt"
...
def log(preventivo, row):
    ...
    file_uno = open(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G02\\log\\{preventivo}.csv", "w")
    file_uno.write(line)
    for data in row:
        file_uno.write(f"\n {data};")
    file_uno.close()
def flussoG02():
    try:
        basepath = r'\\group.local\SHAREDIR\Brescia\V002\DIRCOM\PREVENT\PREVENTIVISTI\FLUSSI_GAUDI\LD_Reti\Registrati'
        for entry in os.listdir(basepath):
            if os.path.isfile(os.path.join(basepath, entry)):
                name = entry
        xmlparse = Xet.parse(r'\\group.local\SHAREDIR\Brescia\V002\DIRCOM\PREVENT\PREVENTIVISTI\FLUSSI_GAUDI\LD_Reti\Registrati\\' + name)
        ...
        for i in root:
            ...
            rows.append((list(i.attrib.values())[0],
                         ...
                         CODICE_POD,
                         REGIME_COMMERCIALE))
        with open('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G12\\G12.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=';')
            line_count = 0
            root = ET.Element("VALIDA_IMPIANTI", COD_SERVIZIO="G02", COD_FLUSSO="0050", TERNA_PIVA="05779661007",
                              GESTORE_PIVA="01341400198")
            ...
            for row in csv_reader:
                ...
                tree.write(
                    f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G02_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml",
...
flussoG02()
gestisci_file_g02()
cerca_file_e_controlla_testo(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti", 'G02', g02_query)
