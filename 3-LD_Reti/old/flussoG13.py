...
def log(preventivo, row):
    ...
    file_uno.close()
def flussoG13():
    basepath = '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\Registrati'
    for entry in os.listdir(basepath):
        if os.path.isfile(os.path.join(basepath, entry)):
            name = entry
    xmlparse = Xet.parse('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\Registrati\\' + name)
    ...
    tree.write(
        f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G13_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml",
        short_empty_elements=False)
...
cerca_file_e_controlla_testo(
                ...
                g13_query)
move_files(r'\\group.local\SHAREDIR\Brescia\V002\DIRCOM\PREVENT\PREVENTIVISTI\FLUSSI_GAUDI\LD_Reti\Registrati', r'\\group.local\SHAREDIR\Brescia\V002\DIRCOM\PREVENT\PREVENTIVISTI\FLUSSI_GAUDI\LD_Reti\Registrati\old')