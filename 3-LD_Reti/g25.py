...
def initialize_logger():
    """Inizializza il logger per G25"""
    global logger
    logger = FlowLogger("G25")
    logger.log_start("Elaborazione flusso G25 - Comunicazione Unica")

def log_legacy(preventivo, row):
    """Mantieni la funzione di log legacy esistente"""
    try:
        line = f'{date.today()};{preventivo}'
        file_uno = open(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G25\\Log\\{preventivo}.csv", "w")
        file_uno.write(line)
        for data in row:
            file_uno.write(f"\n {data};")
        file_uno.close()
...
def flussoG25():
    global logger
    csv_path = '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G25\\G25.csv'
    ...
    try:
        ...
        # Log analisi CSV
        logger.log_csv_analysis(csv_path, processed_records, ['CODICE_POD', 'CODICE_RINTRACCIABILITA', 'DICHIARAZIONE_ACC'])

        # Salva XML
        tree = ET.ElementTree(root)
        output_xml_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G025_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml"
        tree.write(output_xml_path)
        
        # Log XML generato
        logger.log_xml_generated(output_xml_path, processed_records)

    except Exception as e:
        logger.log_error("Errore in flussoG25", e)
        raise
...
def main():
    ...
    try:
        ...
        logger.log_info("Inizio elaborazione flusso G25")
        
        flussoG25()
        
        logger.log_info("Elaborazione G25 completata con successo")
        
    except Exception as e:
        if logger:
            logger.log_error(f"Errore generale nell'esecuzione di G25", e)
        print(f"Errore: {e}")
        raise
    finally:
        if logger:
