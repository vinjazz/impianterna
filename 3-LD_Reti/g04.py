...
def get_most_recent_xml(directory ='\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili'):
    global logger
...
def log_legacy(preventivo, row):
    ...
    try:
        ...
        file_uno = open(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G04\\Log\\{preventivo}.csv", "w")
        file_uno.write(line)
        for data in row:
            file_uno.write(f"\n {data};")
        file_uno.close()
...
def flussoG04():
    ...
    try:
        esercibili_dir = '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili'
        ...
        output_root = ET.Element("ATTIVAZIONE_CONNESSIONE", COD_SERVIZIO="G04", COD_FLUSSO="0050", TERNA_PIVA="05779661007", GESTORE_PIVA="01341400198")
        ...
        output_xml_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\G04_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml"
...
def main():
    ...
    try:
        ...
        move_files('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili',
                   '\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili\\Old')
...
if __name__ == "__main__":
    ...
    initialize_logger()
    flussoG04()
    move_files('\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili','\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\LD_Reti\\1-Esercibili\\Old')
    if logger:
        logger.log_end()
