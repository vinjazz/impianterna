...
def gestisci_file_g02():
    ...
    try:
        directory = "\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Unareti"
...
def g02_query():
    ...
    try:
        ...
        quit;"""
            ...
            filename = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Unareti\\update_query\\G02_update_{timestamp}.txt"
...
def flussoG02():
    ...
    try:
        ...
        basepath = r'\\group.local\SHAREDIR\Brescia\V002\DIRCOM\PREVENT\PREVENTIVISTI\FLUSSI_GAUDI\Unareti\4-Registrati'
        ...
        with open(csv_path) as csv_file:
            ...
            output_root = ET.Element("VALIDA_IMPIANTI", COD_SERVIZIO="G02", COD_FLUSSO="0050", 
                                   TERNA_PIVA="05779661007", GESTORE_PIVA="12883450152")
            ...
            output_xml_path = f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Unareti\\G02_{datetime.now().strftime('%d%m%Y%H%M%S')}.xml"
...
if __name__ == "__main__":
    ...
    flussoG02()
    gestisci_file_g02()
    cerca_file_e_controlla_testo(f"\\\\group.local\\SHAREDIR\\Brescia\\V002\\DIRCOM\\PREVENT\\PREVENTIVISTI\\FLUSSI_GAUDI\\Unareti", 'G02', g02_query)
    if logger:
        logger.log_end()